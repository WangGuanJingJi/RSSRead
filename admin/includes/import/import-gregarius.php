...
class Gregarius_Import extends OPML_Import {
	...
	protected function slimbox ($content) { 
	    $pattern = "/<a(.*?)href=('|\")([^>]*).(bmp|gif|jpeg|jpg|png)([^>]*)('|\")(.*?)>(.*?)<\/a>/i";
	    $replacement = '<a$1href=$2$3.$4$5$6  data-slimbox="slimbox" $7>$8</a>';
	...
	protected function convert_item($old) {
		$new = (object) array(
			...
			'title' => $old->title,
			'content' => $this->slimbox($old->description),
			'summary' => $this->slimbox($old->description),
			'permalink' => $old->url,
