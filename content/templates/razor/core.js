...
RazorUI.init = function () {
	...
	$('#update a,#contentboxupdate').click(function (e) {
		e.preventDefault();
		RazorUI.beginUpdate();
	});
	$('#items-list')
...
RazorUI.openCurrent = function () {
	...
	if (!window.open(current)) {
		alert("你的浏览器居然禁止了弹窗，建议解锁允许弹窗后再试！");
	}
};
...
RazorUI.initializeItemList = function (list) {
	RazorUI.itemCount = 0;
	$('#items-list').empty();
	var li = $('<li id="load-more"><a href="#">更多...</a></li>');
...
RazorUI.populateItemList = function (list) {
	...
	$.each(list, function (id, item) {
		var li = $('<li><a href="#"><span class="item-title" /> <span class="sep">来自</span> <span class="item-source" /> <span class="sep">于</span> <span class="item-date" /></a></li>');
		var a = $('a', li);
	...
	if (RazorUI.itemCount === oldCount) {
		RazorUI.showMessage('没有内容了', 1500);
		$("#load-more").remove();
	}
...
RazorUI.populateItemView = function (item) {
	$('#item-view').empty();
	var basics = $('<div id="item"><div id="heading"><h2 class="item-title"><a target="_blank" /></h2><p class="item-meta"><span class="item-source">来源：<a target="_blank" /></span>. <span class="item-date">时间：<abbr /></span><span class="item-author"> 作者：<a  target="_blank" /></span>.</p></div><div id="item-content" /></div>');
	if (Razor.useFrame) {
		basics = $('<div id="item"><div id="heading"><h2 class="item-title"><a  target="_blank"  /></h2><p class="item-meta"><span class="item-source">来源：<a target="_blank" /></span>. <span class="item-date">时间：<abbr /></span><span class="item-author"> by <a  target="_blank" /></span>.</p></div><iframe id="item-content" class="framed" /></div>');
	...
	$('.item-title a', basics).html(item.title).attr('target', '_blank');
	...
	if (!Razor.useFrame) {
		item.permalink;
		$('#item-content', basics).html(item.content+'<hr class="more-line"><a class="more-permalink" target="_blank" href="'+ item.permalink+'" title="跳转到页面看详细内容" >查看原文</a><br/><br/><br/>');
	}
...
RazorUI.beginUpdate = function () {
	$('#update').hide();
	RazorUI.showMessage('<span id="updating">更新<span class="progress"></span></span>');
	$('#updating').show();
	Razor.feeds = [];
	var feed;
